...
CREATE TABLE IF NOT EXISTS roulette_config (
    ...
    event_id VARCHAR(50),                    -- NULL for 'on_start', required for 'during_event'
    ...
    -- Constraints
    CONSTRAINT chk_roulette_type_event CHECK (
        (roulette_type = 'on_start' AND event_id IS NULL) OR
        (roulette_type = 'during_event' AND event_id IS NOT NULL)
    ),
...
COMMENT ON COLUMN roulette_config.event_id IS 'Event ID for during_event type, NULL for on_start';
